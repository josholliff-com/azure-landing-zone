...
# The identity requirement is driven by the DEFINITION's content, not by the
...
# the problem in their built-ins by shipping "Require a tag on resources" and
# "Add a tag to resources" as two separate definitions rather than one
# parameterized by effect. See ADR 0003.
resource "azurerm_management_group_policy_assignment" "require_tag" {
  ...
  # Required because identity is required. An identity has to live somewhere.
...
#
# Granted at every stage rather than only at Modify. The identity exists from
# the start, so granting early means RBAC propagation has already settled by
# the time the effect flips to Modify, rather than racing it at the moment it
# actually matters.
